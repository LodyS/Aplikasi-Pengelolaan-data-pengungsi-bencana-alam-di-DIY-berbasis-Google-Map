...
function get_tempat_option($selected = ''){
    global $db;
    $rows = $db->get_results("SELECT id_posko, nama_posko FROM tb_tempat ORDER BY id_posko");
    foreach($rows as $row){
        if($row->id_tempat==$selected)
            $a.="<option value='$row->id_posko' selected>$row->nama_posko</option>";
        else
            $a.="<option value='$row->id_posko'>$row->nama_posko</option>";
    }
    return $a;
}
